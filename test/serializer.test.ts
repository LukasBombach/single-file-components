...
test('an element', async () => {
  ...
  expect(ReactSerializer.serialize(json)).toEqual(`React.createElement(div, {}, ["contents"])`)
});

test('an element with a prop', async () => {
  const json = Parser.getJson(`<template><div foo="bar">contents</div></template>`);
  expect(ReactSerializer.serialize(json)).toEqual(`React.createElement(div, {"foo":"bar"}, ["contents"])`)
});

test('an element with a child element', async () => {
  const json = Parser.getJson(`<template><div><div>contents</div></div></template>`);
  expect(ReactSerializer.serialize(json)).toEqual(`React.createElement(div, {}, [React.createElement(div, {}, ["contents"])])`)
});